...
| **10** — auto-logging + pesos negativos permitidos (500 pasos/iter) | Rama `bounded-features-no-sign-clip` con auto-logging funcional. 500 pasos/iter. Ahora capturamos el log completo con `mu_experto` y `mu^(i)` de cada iteración. **Resultado clave**: pesos SÍ pueden ser negativos y se guardan correctamente (no son recortados a 0). | `[-0.063, -0.773, 0, 0.066, -0.456, 0.185, 0.165, 0.324]` | **21.43** ← **mejor margen aún** | 100–235 pasos | Auto-logging confirma que el pipeline de pesos funciona: negativos se guardan, no se recortan. Margen mejoró significativamente (21.43 vs 23.59). Pesos negativos en `proximidad_objetivo`, `estabilidad_altura` y `oscilacion` indican que el algoritmo SÍ puede usar pesos negativos cuando `restringir_w()` los permite (sin el clip `w>=0`). |
